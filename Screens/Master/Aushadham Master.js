...
export default function Aushadham() {
    ...
    const [id, setId] = useState('M01')
    const [change, setChange] = useState(null)
    const [filteredAushadhamData, setFilteredAushadhamData] = useState([])
    ...
    const currentPosts = filteredAushadhamData.slice(indexOfFirstItem, indexOfLastItem)
    ...
    useEffect(() => {
        ...
        //     tx.executeSql(`SELECT * FROM Aushadham ORDER BY Name`, [], (_, { rows: { _array } }) => {
        //         setPageNumbers([])
        //         setAushadhamData(_array);
        //         setFilteredAushadhamData(_array);
        //         setIsloading(false)
        ...
        //         tx.executeSql(`INSERT INTO Aushadham (
        //             Id,
        //             Aushadham
        //         )
        ...
        db.transaction((tx) => {
            if (len < 1) {
                for (let i = 0; i < Medicine.length; i++) {
                    tx.executeSql(`INSERT INTO Aushadham(Id,Aushadham) VALUES(?,?)`, [`M0${i + 1}`, Medicine[i]]
    ...
    const handleDelete = (rowMap, rowKey) => {
        ...
        db.transaction((tx) => {
            tx.executeSql(`UPDATE Receipt SET Aushadham=null WHERE Aushadham=?`, [rowKey])
        })

        db.transaction((tx) => {
            tx.executeSql(`DELETE FROM Aushadham WHERE ID=?`, [rowKey])
        })
        setChange(rowKey)
    }
    ...
    const handleEdit = (data, rowMap, rowKey) => {
        ...
        setIndex(AushadhamData.findIndex((item) => item.Id == rowKey))
    }

    const searchData = (val) => {
        if (val) {
            const newData = AushadhamData.filter(function (item) {
            ...
            setFilteredAushadhamData(newData);
            setSearch(val);
        } else {
            setFilteredAushadhamData(AushadhamData);
            setSearch(val);
        }
    };
    ...
    const addUser = () => {
        ...
        setSelectedData([]);
        if (AushadhamData != 0) {
            setId("M0" + `${parseInt(AushadhamData[AushadhamData.length - 1].Id.slice(2)) + 1}`)
        }
    }
                            ...
                            renderItem={(data) => {
                                return (
                                    <View style={[styles.item, { backgroundColor: colors.crd }]}>
                                        ...
                                        <Text style={[styles.name, { color: colors.txt, fontSize: colors.font }]}>{data.item.Aushadham}</Text>
                                    </View>
                                )
                            }}
                ...
                <View style={{ flex: 1, backgroundColor: colors.back }}>
                    <Formik
                        ...
                        onSubmit={(values) => {
                            if (selected) {
                                db.transaction((tx) => {
                                    tx.executeSql(`UPDATE Aushadham SET Aushadham=? WHERE Id=?`, [values.Name, values.Id],
