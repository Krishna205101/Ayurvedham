...
export default function Laxanam() {

    const { colors } = useTheme()

    const [LaxanamData, setLaxanamData] = useState([])
    const [modalVisible, setModalVisible] = useState(false)
    const [selectedData, setSelectedData] = useState([])
    const [selected, setSelected] = useState(false)
    const [index, setIndex] = useState(0)
    const width = Dimensions.get('window').width
    const [id, setId] = useState('SS01')
    const [change, setChange] = useState(null)
    const [filteredLaxanamData, setFilteredLaxanamData] = useState([])
    ...
    const currentPosts = filteredLaxanamData.slice(indexOfFirstItem, indexOfLastItem)
    ...
    useEffect(() => {
        ...
        db.transaction((tx) => {
            tx.executeSql(`SELECT * FROM Laxanam`, [], (_, { rows: { _array } }) => {
                setPageNumbers([])
                setLaxanamData(_array);
                setFilteredLaxanamData(_array);
                setIsloading(false)
        ...
        db.transaction((tx) => {
            if (len < 1) {
                ...
                for (let i = 0; i < Roga.length; i++) {
                    tx.executeSql(`INSERT INTO Laxanam(Id,Name) VALUES(?,?)`, [`SS0${i + 1}`, Roga[i]])
                }
            }
        })
    ...
    const handleDelete = (rowMap, rowKey) => {
        db.transaction((tx) => {
            tx.executeSql(`DELETE FROM PresLaxanam WHERE Laxanam=?`, [rowKey], (data) => console.log('data' + data), (err) => console.log('error' + err))
        })

        db.transaction((tx) => {
            tx.executeSql(`DELETE FROM Laxanam WHERE ID=?`, [rowKey], (data) => console.log('data' + data), (err) => console.log('error' + err))
        })
        setChange(rowKey)
    }
    ...
    const handleEdit = (data, rowMap, rowKey) => {
        ...
        setIndex(LaxanamData.findIndex((item) => item.Id == rowKey))
    }

    const searchData = (val) => {
        if (val) {
            const newData = LaxanamData.filter(function (item) {
            ...
            setFilteredLaxanamData(newData);
            setSearch(val);
        } else {
            setFilteredLaxanamData(LaxanamData);
            setSearch(val);
        }
    };
    ...
    const addUser = () => {
        ...
        setSelectedData([]);
        if (LaxanamData != 0) {
            setId("SS0" + `${parseInt(LaxanamData[LaxanamData.length - 1].Id.slice(2)) + 1}`)
        }
    }
        ...
        <View style={[styles.container, { backgroundColor: colors.back }]}>
            ...
            <View style={styles.list}>
                <View style={styles.header}>
                    <Text style={[styles.title, { color: colors.txt, fontSize: colors.font + 5 }]}>Signs & Symptoms Screen</Text>
                ...
                <View style={{ height: 450 }}>
                    ...
                    <View style={styles.pagination}>
                        ...
                        <Text style={{ color: colors.txt, fontSize: colors.font + 3 }}>{currentPage}</Text>
                ...
                <View style={{ flex: 1, backgroundColor: colors.back }}>
                    <Formik
                        ...
                        onSubmit={(values) => {
                            if (selected) {
                                db.transaction((tx) => {
                                    tx.executeSql(`UPDATE Laxanam SET Name=? WHERE Id=?`, [values.Name, values.Id],
                                ...
                                db.transaction((tx) => {
                                    tx.executeSql(`INSERT INTO Laxanam (
                                    Id,
                                    Name
                                )
                            ...
                            <View>
                                ...
                                <View style={styles.form}>
                                    <View style={styles.field}>
                                        <Text style={[styles.heading, { color: colors.txt, fontSize: colors.font }]}>Laxanam Id</Text>
                                    ...
                                    <View style={[styles.field, { justifyContent: "center", alignItems: "center" }]}>
                                        <Text style={[styles.heading, { color: colors.txt, fontSize: colors.font }]}>Laxanam Name</Text>
                                        <TextInput style={[styles.input, { backgroundColor: 'white' }]} placeholder="Laxanam Name" onChangeText={props.handleChange('Name')} value={props.values.Name} />
                                    </View>
