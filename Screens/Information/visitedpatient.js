...
export default function VisitedPatients({ navigation }) {
    ...
    useEffect(() => {
        ...
        db.transaction((tx) => {
            ...
            if (date.getDate() < 10) {
                tx.executeSql(`SELECT * FROM Patient WHERE VisitedDate LIKE '%${month[date.getMonth()] + " 0" + date.getDate() + " " + date.getFullYear()}%'`, [], (_, { rows: { _array } }) => {
                ...
                tx.executeSql(`SELECT * FROM Patient WHERE VisitedDate LIKE '%${dateValue}%'`, [], (_, { rows: { _array } }) => {
                    setPatientData(_array);
                    setFilteredPatientData(_array)
                    setIsloading(false)
                    setPageNumbers(Math.ceil(_array.length / pageItems))
    ...
    const set = (val) => {
        ...
        if (val.type == "set") {
            ...
            db.transaction((tx) => {
                tx.executeSql(`SELECT * FROM Patient WHERE VisitedDate LIKE '%${visitedDate}%'`, [], (_, { rows: { _array } }) => {
                    // console.log(_array)
                    setFilteredPatientData(_array)
                    setPatientData(_array);
                })
        ...
        <View style={[styles.container, { backgroundColor: colors.back }]}>
            ...
            <View style={styles.list}>
                <View style={styles.header}>
                    <Text style={[styles.title, { color: colors.txt, fontSize: colors.font + 5 }]}>Visited Patients List</Text>
