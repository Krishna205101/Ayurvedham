...
export default function Form({ title, save, prescription, reset }) {
        ...
        if (save) {
            db.transaction((tx) => {
                for (let i = 0; i < selectedData.length; i++) {
                    tx.executeSql(`INSERT INTO PresRoga (
                        Prescription,
                        Roga
                    )
                    VALUES (?,?);`, [prescription, selectedData[i].Id],
                        // (data) => { console.log(`we made it Roga`) }, (err) => console.log(`We have encounter an Error in Roga`)
                    )
                }
                setSelectedData([])
            })
    ...
    const insert = (Id) => {
        db.transaction((tx) => {
            tx.executeSql(`INSERT INTO Roga VALUES(?,?)`, [Id, search], (data => setSearch("")))
        })
    ...
    const addItem = () => {
        if (search) {
            if (dropdownData != 0) {
                insert(`D0` + `${parseInt(dropdownData[dropdownData.length - 1].Id.slice(2)) + 1}`)
                insert
            }
            else {
                insert(`D01`)
    ...
    return (
        <View style={styles.main}>
            <Text style={{ color: colors.txt, fontSize: colors.font + 5, fontWeight: "bold" }}>Main Complaint</Text>
