...
export default function Form({ title, save, prescription, reset }) {
            ...
            db.transaction((tx) => {
                for (let i = 0; i < selectedData.length; i++) {
                    ...
                    VALUES (?,?);`, [prescription, selectedData[i].Id],
                        // (data) => { console.log(`we made it Laxanam`) }, (err) => console.log(`We have encounter an Error in Laxanam`)
                    )
                }
                setSelectedData([])
            })
    ...
    const insert = (Id) => {
        db.transaction((tx) => {
            tx.executeSql(`INSERT INTO Laxanam VALUES(?,?)`, [Id, search], (data => setSearch("")))
        })
    ...
    const addItem = () => {
        if (search) {
            if (dropdownData != 0) {
                insert(`SS0` + `${parseInt(dropdownData[dropdownData.length - 1].Id.slice(2)) + 1}`)
                insert
            }
            else {
                insert(`SS01`)
    ...
    return (
        <View style={styles.main}>
            <Text style={{ color: colors.txt, fontSize: colors.font + 5, fontWeight: "bold" }}>Signs & Symptoms</Text>
